...
const PrintPaymentOutModal = ({ paymentId, open, onClose }) => {
        ...
        const fetchData = async () => {
            ...
            try {
                const [paymentRes, companyRes] = await Promise.all([
                    fetch(process.env.REACT_APP_API_URL + `/paymentout/get`, {
                        method: "POST",
            ...
            <Modal.Body>
                {
                    loading ? (
                        ...
                        <>
                            ...
                            <div id='paymentOutReceipt' className='border rounded p-4'>
                                        ...
                                        <div className='border mt-3'>
                                            <div className='flex border-b'>
                                                ...
                                                <div className='flex flex-col justify-center px-3' style={{ fontSize: '12px', width: '40%' }}>
                                                    <p><span className='font-semibold'>Receipt No: </span>{paymentData.paymentOutNumber}</p>
                                                    <p><span className='font-semibold'>Date: </span>{new Date(paymentData.paymentOutDate).toLocaleDateString()}</p>
                                                </div>
                                            </div>

                                            {/* Party Details */}
                                            <div className='p-3 border-b'>
                                                <p style={{ fontSize: '12px' }}>PAID TO</p>
                                                        ...
                                                        <table className='w-full' style={{ fontSize: '12px' }}>
                                                            <thead className='bg-gray-100'>
                                                                <tr>
                                                                    ...
                                                                    <td className='p-2 font-semibold border'>Amount Paid</td>
                                                                </tr>
                                                            </thead>
                                                            <tbody>
                                                                {
                                                                    paymentData.sattleInvoice.map((inv, i) => (
                                                                        <tr key={i}>
                                                                            <td className='p-2 border'>{inv.purchaseInvoiceNumber}</td>
                                            ...
                                            <div className='p-3 bg-gray-50 flex justify-end'>
                                                <div className='text-right'>
                                                    <p className='font-semibold' style={{ fontSize: '12px' }}>Total Amount Paid</p>
...
export default PrintPaymentOutModal;
