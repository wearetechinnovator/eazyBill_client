...
const DeliveryChalan = ({ mode }) => {
	...
	const [formData, setFormData] = useState({
		...
		items: ItemRows, additionalCharge: additionalRows, note: '', terms: ``, discountType: '', discountAmount: '',
	...
	const get = async () => {
		try {
			...
			setFormData({
				...formData, ...res.data,
				chalanDate: res.data.chalanDate ? new Date(res.data.chalanDate).toISOString().split('T')[0] : '',
				validDate: res.data.validDate ? new Date(res.data.validDate).toISOString().split('T')[0] : '',
			});
	...
	// Get API data for item, unit, tax, party
	useState(() => {
		const apiData = async () => {
			{
