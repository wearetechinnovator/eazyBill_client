...
import { SelectPicker, Button, TagPicker } from 'rsuite';
...
import MyTagPicker from '../../components/MyTagPicker';
...
const SalesInvoice = ({ mode }) => {
	...
	// Item wise store purchase invoice for TagPicker;
	const [settleInvoice, setSettleInvoice] = useState([]);
	// Item wise store all purchase invoice;
	const [allSettleInvoice, SetAllSettleInvoice] = useState([]);
	...
	const getPurchaseInvoice = async (itemId) => {
		...
		try {
			let URL = `${process.env.REACT_APP_API_URL}/item/get-purchase-invoice`;
												...
												<div className='flex flex-col gap-1'>
													...
													<MyTagPicker
														...
														onChange={(selected) => {
															...
															setItemRows((p) => {
																const prevItems = [...p];
																prevItems[index].settleInvoice = values;
																return prevItems;
															});
														}}
													/>
